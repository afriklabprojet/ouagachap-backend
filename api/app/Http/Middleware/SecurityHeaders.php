...
class SecurityHeaders
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->is('admin') || $request->is('admin/*') || $request->is('livewire/*')) {
            // Relaxed CSP for Filament admin panel (Alpine.js requires unsafe-eval, Leaflet + Google Maps)
            $response->headers->set('Content-Security-Policy', implode('; ', [
                "default-src 'self'",
                "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://unpkg.com https://maps.googleapis.com https://maps.gstatic.com",
                ...
                "connect-src 'self' wss: ws: https://*.tile.openstreetmap.org https://maps.googleapis.com",
