...
class CacheService
{
    ...
    public function getGeneralConfig(): array
    {
        return Cache::remember('config:general', $this->defaultTtl, function () {
            return [
                ...
                'support_email' => 'support@ouagachap.pro',
                'support_whatsapp' => '+22670000000',
                'currency' => config('app.currency', 'XOF'),
                'country_code' => 'BF',
                'default_language' => 'fr',
                'commission_rate' => optional(\App\Models\Zone::active()->first())->commission_rate ?? config('app.commission_rate', 0.15),
    ...
    public function getLandingSettings(): array
    {
        return Cache::remember('site_settings:landing', $this->defaultTtl, function () {
            ...
            // Décoder les JSON
            foreach (['features', 'pricing_plans', 'pricing', 'testimonials', 'how_it_works_steps'] as $key) {
     ...
     * - Changement de status d'une commande
     * - Changement de status d'un coursier
     */
    public function clearStatsCache(): void
    {
        Cache::forget('stats:general');
        Cache::forget('stats:dashboard');
        Cache::forget('stats:orders');
        Cache::forget('stats:revenue');
    }
