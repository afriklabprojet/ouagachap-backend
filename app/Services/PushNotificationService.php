...
class PushNotificationService
{
    ...
    public function notifyOrderCreated(Order $order): bool
    {
        return $this->sendToUser(
            $order->client,
            '🎉 Commande créée',
            "Votre commande #{$order->tracking_number} a été créée. Nous recherchons un coursier.",
            [
                'type' => 'order_created',
                'order_id' => $order->id,
    ...
    public function notifyOrderDelivered(Order $order): bool
    {
        return $this->sendToUser(
            $order->client,
            '✅ Livraison effectuée',
            "Votre colis a été livré à {$order->recipient_name}. Merci d'utiliser OUAGA CHAP!",
            [
    ...
    public function notifyOrderCancelled(Order $order): bool
    {
        ...
        return $this->sendToUser(
            $order->courier,
            '❌ Course annulée',
            "La course #{$order->tracking_number} a été annulée par le client.",
            [
    ...
    public function notifyPaymentReceived(Order $order): bool
    {
        return $this->sendToUser(
            $order->client,
            '💰 Paiement confirmé',
            "Paiement de {$order->total_amount} FCFA reçu pour la commande #{$order->tracking_number}",
            [
